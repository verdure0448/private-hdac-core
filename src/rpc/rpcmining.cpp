...
#ifdef FEATURE_HDAC_DISABLE_EPOW
    return (int64_t)0;
#else
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Hdac is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "HDac	 is downloading blocks...");
...
Value estimatefee(const Array& params, bool fHelp)
{
    ...
    if (nBlocks < 1)
        nBlocks = 1;
//  int nBytes = params[0].get_int();
//  if (nBytes < 1000)
//      nBytes = 1000;
//  int nBlocks = nBytes / 1000 + 1;

    CFeeRate feeRate = mempool.estimateFee(nBlocks);
    if (feeRate == CFeeRate(0))
        return -1.0;
//      return ValueFromAmount((double)MIN_RELAY_TX_FEE * (double)nBytes / 1000.0);

    return ValueFromAmount(feeRate.GetFeePerK());
//  return ValueFromAmount(feeRate.GetFeePerK() * (double)nBytes / 1000.0);
}
