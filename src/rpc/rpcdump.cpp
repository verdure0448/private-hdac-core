...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        ...
        if (address.IsValid()) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Hdac address or script");
        }
...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    while (pindex && pindex->pprev && pindex->GetBlockTime() > nTimeBegin - 12 * Params().TargetSpacing())
        pindex = pindex->pprev;
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Hdac address");
    CKeyID keyID;
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Hdac %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
