...
//        int period_pos=permission_type.find_last_of(".",permission_type.size());
    ...
    for(int i=0;i<permissions->GetCount();i++)
    {
        ...
        switch(plsRow->m_Type)
        {
            ...
            case MC_PTP_GETSTAT:entry.push_back(Pair("type", "getstatus"));break;
            case MC_PTP_CONTROL:entry.push_back(Pair("type", "control"));break;
            case MC_PTP_EXECUTE:entry.push_back(Pair("type", "execute"));break;
