...
//              included custfile
// 2018/05/00	Add getaddresses type option
//		getaddresses [verbose [type]]
//		0=all addresses, 1=local addresses (has private key), 2=remote addresses
//============================================================================================
...
Value getaddresses(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 1)
    {
        if(params[1].type() == int_type)
        {
            addr_type = params[1].get_int();
        }
        else if(params[1].type() == str_type)
        {
            addr_type = atoi(params[1].get_str());
        }
        else
        {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid value for 'address type' parameter, should be numeric");                                                                
        }        
    }
        ...
        for(int i=0;i<entity_count;i++)
        {
            ...
            if( (lpEntity->m_Entity.m_EntityType & MC_TET_ORDERMASK) == MC_TET_CHAINPOS)
            {
                if(CBitcoinAddressFromTxEntity(address,&(lpEntity->m_Entity)))
                {
                    CTxDestination dest = address.Get();
                    isminetype mine = pwalletMain ? IsMine(*pwalletMain, dest) : ISMINE_NO;
                    bool islocal = (mine & ISMINE_SPENDABLE) ? true : false;

                    if (islocal && addr_type == 2)	// remote
		    	continue;
                    if (!islocal && addr_type == 1)	// local
		    	continue;
