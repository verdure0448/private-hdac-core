...
int CheckRequiredPermissions(const CTxDestination& addressRet,int expected_allowed,map<uint32_t, uint256>* mapSpecialEntity, string *strFailReason)
{
    ...
    if(aptr)
    {
        ...
        if(expected_allowed & MC_PTP_GETSTAT)
        {
            unsigned char *lpEntity=NULL;
            if(mapSpecialEntity)
            {
            ...
            if(mc_gState->m_Permissions->CanGetStat(lpEntity,aptr))
            {
                allowed |= MC_PTP_GETSTAT;
            }                                                 
            if(strFailReason)
            {
                if( (allowed & MC_PTP_GETSTAT) == 0 )
                {
                    *strFailReason="Get status in this stream is not allowed from this address";
                }
            }
        }
        if(expected_allowed & MC_PTP_CONTROL)
        {
            unsigned char *lpEntity=NULL;
            if(mapSpecialEntity)
            {
            ...
            if(mc_gState->m_Permissions->CanControl(lpEntity,aptr))
            {
                allowed |= MC_PTP_CONTROL;
            }                                                 
            if(strFailReason)
            {
                if( (allowed & MC_PTP_CONTROL) == 0 )
                {
                    *strFailReason="Control in this stream is not allowed from this address";
                }
            }
        }
        if(expected_allowed & MC_PTP_EXECUTE)
        {
            unsigned char *lpEntity=NULL;
            if(mapSpecialEntity)
            {
            ...
            if(mc_gState->m_Permissions->CanExecute(lpEntity,aptr))
            {
                allowed |= MC_PTP_EXECUTE;
            }                                                 
            if(strFailReason)
            {
                if( (allowed & MC_PTP_EXECUTE) == 0 )
                {
                    *strFailReason="Execute in this stream is not allowed from this address";
                }
            }
        }
