...
boost::filesystem::path mc_GetDefaultDataDir()
{
    ...
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "HdacCold";
    }
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"hdac",".conf") == 0)
    {
                    ...
                    const char *prefix)
{
    try
    {
        ...
        if (!streamConfig.good())
            return MC_ERR_NOERROR; // No hdac.conf file is OK
...
{
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"hdac",".conf"),&mapArgs, &mapMultiArgs,"-");
...
int mc_HdacParams::SetGlobals()
{
    ...
    while(MAX_BLOCK_SIZE>MAX_BLOCKFILE_SIZE)	// > 256MB ?
    {
        MAX_BLOCKFILE_SIZE *= 2;
    }
    while(MAX_BLOCK_SIZE>MAX_SIZE)	// > 64MB ?
    {
        MAX_SIZE *= 2;
    }
    ...
    MAX_MONEY=1000000000000000000;	// for public
