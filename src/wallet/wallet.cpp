...
// 2018/06/08	Change rescaning progress display
//============================================================================================
...
string CWallet::SetDefaultKeyIfInvalid(std::string init_privkey)
{
    ...
    if(!vchSecret.SetString(init_privkey))
    {
        ...
        {
            return "Private key version is not set, please chose seed node running at least Hdac 1.0 beta 2";
        }
...
void CWallet::SyncTransaction(const CTransaction& tx, const CBlock* pblock)
{
    ...
    {
        LogPrint("hdac","hdac: Wallet optimization after block\n");
        fEmpty=true;
    }
 ...
 */
        ...
        while (pindex)
        {
            ...
            if(!fOnlyUnsynced)
            {
                ...
                {
                    printf(" %d of %d blocks rescanned    \n",pindex->nHeight,chainActive.Height()); fflush(stdout);
                }
            //  if((pindex->nHeight % 10) == 0)
                {
                    printf(" %d of %d blocks rescanned    \r",pindex->nHeight,chainActive.Height()); fflush(stdout);
                }
            }
            pindex = chainActive.Next(pindex);
            if (GetTime() >= nNow + 60) {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-hdac-address
...
{
    {
        ...
        if (!wtxNew.AcceptToMemoryPoolReturnReason(false,true,reject_reason))
        {
            ...
            if(address.GetKeyID(keyID))
            {
                ...
                if(perm == type)
                {
                    ...
                    if(mapSpecialEntity)
                    {
                        ...
                        if(type & MC_PTP_GETSTAT)
                        {
                            unsigned char *lpEntity=NULL;

                            std::map<uint32_t,uint256>::const_iterator it = mapSpecialEntity->find(MC_PTP_GETSTAT);
                            if (it != mapSpecialEntity->end())
                            {
                                lpEntity=(unsigned char*)(&(it->second));
                            }
                            take_it=false;
                            if(mc_gState->m_Permissions->CanGetStat(lpEntity,(unsigned char*)(&keyID)))
                            {
                                take_it=true;
                            }
                        }
                        if(type & MC_PTP_CONTROL)
                        {
                            unsigned char *lpEntity=NULL;

                            std::map<uint32_t,uint256>::const_iterator it = mapSpecialEntity->find(MC_PTP_CONTROL);
                            if (it != mapSpecialEntity->end())
                            {
                                lpEntity=(unsigned char*)(&(it->second));
                            }
                            take_it=false;
                            if(mc_gState->m_Permissions->CanControl(lpEntity,(unsigned char*)(&keyID)))
                            {
                                take_it=true;
                            }
                        }
                        if(type & MC_PTP_EXECUTE)
                        {
                            unsigned char *lpEntity=NULL;

                            std::map<uint32_t,uint256>::const_iterator it = mapSpecialEntity->find(MC_PTP_EXECUTE);
                            if (it != mapSpecialEntity->end())
                            {
                                lpEntity=(unsigned char*)(&(it->second));
                            }
                            take_it=false;
                            if(mc_gState->m_Permissions->CanExecute(lpEntity,(unsigned char*)(&keyID)))
                            {
                                take_it=true;
                            }
                        }
...
void CWallet::GetKeyBirthTimes(std::map<CKeyID, int64_t> &mapKeyBirth) const {
    ...
    else
    {
        ...
        for (std::map<CKeyID, CBlockIndex*>::const_iterator it = mapKeyFirstBlock.begin(); it != mapKeyFirstBlock.end(); it++)
            mapKeyBirth[it->first] = it->second->GetBlockTime() - 12 * Params().TargetSpacing();	// block times can be 2h off
    }
