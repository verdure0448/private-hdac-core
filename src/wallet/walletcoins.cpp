                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse,uint32_t flags,int *eErrorCode)
{
    ...
    {
        if (nValue < 0)                                                         // Hdac allows protocol zero-value outputs
        {
    ...
    int special_required=MC_PTP_ADMIN | MC_PTP_ACTIVATE | MC_PTP_ISSUE | MC_PTP_CREATE | MC_PTP_WRITE | MC_PTP_GETSTAT | MC_PTP_CONTROL | MC_PTP_EXECUTE;
#else
    ...
    if(required & MC_PTP_GETSTAT)
    {
        in_special_row[8]=in_amounts->GetCount();
        memset(in_row,0,in_size);
        type=MC_PTP_GETSTAT | MC_PTP_SEND;
        mc_PutLE(in_row+4,&type,4);
        mc_SetABRefType(in_row,MC_AST_ASSET_REF_TYPE_SPECIAL);
        in_amounts->Add(in_row);
    }

    if(required & MC_PTP_CONTROL)
    {
        in_special_row[8]=in_amounts->GetCount();
        memset(in_row,0,in_size);
        type=MC_PTP_CONTROL | MC_PTP_SEND;
        mc_PutLE(in_row+4,&type,4);
        mc_SetABRefType(in_row,MC_AST_ASSET_REF_TYPE_SPECIAL);
        in_amounts->Add(in_row);
    }

    if(required & MC_PTP_EXECUTE)
    {
        in_special_row[8]=in_amounts->GetCount();
        memset(in_row,0,in_size);
        type=MC_PTP_EXECUTE | MC_PTP_SEND;
        mc_PutLE(in_row+4,&type,4);
        mc_SetABRefType(in_row,MC_AST_ASSET_REF_TYPE_SPECIAL);
        in_amounts->Add(in_row);
    }
                                ...
                                if(mc_GetABRefType(out_amounts->GetRow(asset)) == MC_AST_ASSET_REF_TYPE_SPECIAL)
                                {
                                    switch(mc_GetLE(out_amounts->GetRow(asset)+4,4))
                                    {
                                        ...
                                        case MC_PTP_WRITE | MC_PTP_SEND:
                                            ...
                                            break;
#ifdef FEATURE_HDAC_EXTENDED_PERMISSIONS
                                        case MC_PTP_GETSTAT | MC_PTP_SEND:
                                            if(eErrorCode)*eErrorCode=RPC_INSUFFICIENT_PERMISSIONS;
                                            strFailReason = _("No unspent output with getstatus permission");
                                            break;
                                        case MC_PTP_CONTROL | MC_PTP_SEND:
                                            if(eErrorCode)*eErrorCode=RPC_INSUFFICIENT_PERMISSIONS;
                                            strFailReason = _("No unspent output with control permission");
                                            break;
                                        case MC_PTP_EXECUTE | MC_PTP_SEND:
                                            if(eErrorCode)*eErrorCode=RPC_INSUFFICIENT_PERMISSIONS;
                                            strFailReason = _("No unspent output with execute permission");
                                            break;
#endif	// FEATURE_HDAC_EXTENDED_PERMISSIONS
                                        case MC_PTP_SEND:
                                            if(nTotalOutValue > 0)
        ...
        if(!result)
        {
            ...
            if (it != addressesToOptimize.end())
            {
                if(txOutCounts[it->second] >= min_outputs)
                {
                    ...
                    CBitcoinAddress hdac_address=CBitcoinAddress(*lpKeyID);
                    if(lpKeyID)
                    {
                        CWalletTx wtx;
