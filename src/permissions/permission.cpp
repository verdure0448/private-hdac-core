...
// 2018/03/00	FEATURE_HDAC_EXTENDED_PERMISSIONS added
//		Rename APIs
//============================================================================================
    ...
    for(i=from;i<to;i++)
    {
        ...
        switch(row->m_Type)
        {
            ...
            case MC_PTP_WRITE:
                ...
                break;
#ifdef FEATURE_HDAC_EXTENDED_PERMISSIONS
            case MC_PTP_GETSTAT:
                if(CanGetStat(row->m_Entity,row->m_Address) == 0)
                {
                    return 0;
                }
                break;
            case MC_PTP_CONTROL:
                if(CanControl(row->m_Entity,row->m_Address) == 0)
                {
                    return 0;
                }
                break;
            case MC_PTP_EXECUTE:
                if(CanExecute(row->m_Entity,row->m_Address) == 0)
                {
                    return 0;
                }
                break;
#endif	// FEATURE_HDAC_EXTENDED_PERMISSIONS
        }
    }
...
#ifdef FEATURE_HDAC_EXTENDED_PERMISSIONS
            full_type = MC_PTP_WRITE | MC_PTP_ACTIVATE | MC_PTP_ADMIN | MC_PTP_GETSTAT | MC_PTP_CONTROL | MC_PTP_EXECUTE;
#else
...
#ifdef FEATURE_HDAC_EXTENDED_PERMISSIONS
                if(mc_MemcmpCheckSize(start,"getstatus", ptr-start) == 0)perm_type = MC_PTP_GETSTAT;
                if(mc_MemcmpCheckSize(start,"control", ptr-start) == 0)perm_type = MC_PTP_CONTROL;
                if(mc_MemcmpCheckSize(start,"execute", ptr-start) == 0)perm_type = MC_PTP_EXECUTE;
#endif	// FEATURE_HDAC_EXTENDED_PERMISSIONS
...
#ifdef FEATURE_HDAC_EXTENDED_PERMISSIONS
    if(type & MC_PTP_GETSTAT)    result |= CanGetStat(lpEntity,lpAddress);
    if(type & MC_PTP_CONTROL)    result |= CanControl(lpEntity,lpAddress);
    if(type & MC_PTP_EXECUTE)    result |= CanExecute(lpEntity,lpAddress);
#endif	// FEATURE_HDAC_EXTENDED_PERMISSIONS
...
int mc_Permissions::CanGetStat(const void* lpEntity,const void* lpAddress)
{
    int result;
    mc_MempoolPermissionRow row;
    ...
    result = GetPermission(lpEntity,lpAddress,MC_PTP_GETSTAT);    
    
    if(result)
    {
        result = MC_PTP_GETSTAT; 
    }
...
int mc_Permissions::CanControl(const void* lpEntity,const void* lpAddress)
{
    int result;
    mc_MempoolPermissionRow row;
    ...
    result = GetPermission(lpEntity,lpAddress,MC_PTP_CONTROL);    
    
    if(result)
    {
        result = MC_PTP_CONTROL; 
    }
...
int mc_Permissions::CanExecute(const void* lpEntity,const void* lpAddress)
{
    int result;
    mc_MempoolPermissionRow row;
    ...
    result = GetPermission(lpEntity,lpAddress,MC_PTP_EXECUTE);    
    
    if(result)
    {
        result = MC_PTP_EXECUTE; 
    }
...
#ifdef FEATURE_HDAC_EXTENDED_PERMISSIONS
    types[num_types]=MC_PTP_GETSTAT;	num_types++;        
    types[num_types]=MC_PTP_CONTROL;	num_types++;        
    types[num_types]=MC_PTP_EXECUTE;	num_types++;        
#endif	// FEATURE_HDAC_EXTENDED_PERMISSIONS
