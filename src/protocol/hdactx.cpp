...
// 2018/06/08	Add AddAddressToWallet()
//============================================================================================
        ...
        for (unsigned int j = 0; j < tx.vout.size(); j++)
        {
            ...
            else                                                                // Not OP_RETURN
            {
                        ...
                        if(mc_gState->m_TmpScript->GetPermission(&type,&from,&to,&timestamp) == 0)// Permission script
                        {
                            ...
                            switch(pass)
                            {
                                case 0:                                         // Not admin or activate
                                    ...
                                    type &= ( MC_PTP_CONNECT | MC_PTP_SEND | MC_PTP_RECEIVE | MC_PTP_WRITE | MC_PTP_BANNED | MC_PTP_GETSTAT | MC_PTP_CONTROL | MC_PTP_EXECUTE);
#else
                ...
                if(pass == 3)
                {
                    ...
                    {
			reason="Script rejected - ExtractOutputAssetQuantities failed";
                        fReject=true;
                        goto exitlbl;                                                                        
                    }
                }
...
exitlbl:
    ...
    if(fReject)
    {
        if(fDebug)LogPrint("hdac","hdac: Tx rejected: reason=%s: %s\n", reason, EncodeHexTx(tx));
    }
